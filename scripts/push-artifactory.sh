...
if [ "${TRAVIS_BRANCH}" = "master" ] && [ "${TRAVIS_TAG}" ]; then
  ...
  docker tag -f "${DOCKER_REGISTRY_DEV}/${NAME}:${TRAVIS_BRANCH}-${COMMIT}" "${DOCKER_REGISTRY_DEV}/${NAME}:${TRAVIS_TAG}"
  docker push "${DOCKER_REGISTRY_DEV}/${NAME}:${TRAVIS_TAG}"

  # Tag & Push 'latest' tag to Development Registry
  docker tag -f "${DOCKER_REGISTRY_DEV}/${NAME}:${TRAVIS_BRANCH}-${COMMIT}" "${DOCKER_REGISTRY_DEV}/${NAME}:latest"
